...
- Packet-level HTSim source pacing now uses the bottleneck bandwidth of the selected cached route instead of the source node's first adjacency. Same-server traffic is paced at the scale-up route rate (3600 Gbps in the current topologies); cross-server traffic is paced at 400 Gbps on Meta and 200 Gbps per selected HPN uplink, matching NS-3's per-device QP-rate model.
- Use `-r ecmp` when a flow must select one shortest path and remain pinned to it. `spray_plb` has only one active path at a time, but may switch that path after congestion feedback; it does not send a flow over both HPN uplinks simultaneously.
- HTSim `RoceSink` now retains out-of-order data as merged byte intervals. When a missing packet arrives, the cumulative ACK advances across all contiguous buffered intervals instead of discarding already received packets and forcing the source to retransmit the entire suffix. Existing NACK behavior is retained for each observed gap.
- HTSim `spray_plb` keeps normal-stage tail RTO disabled and detects post-pass event-loop livelock from the absence of both flow completions and cumulative ACK progress. It hands only currently active Go-Back-N sources to ACK-gated oldest-unacked recovery. Flows created later in the same recovery campaign send one initial packet before joining controlled recovery, so they are never frozen at cumulative ACK zero.
- `HTSIM_FINAL_DRAIN_RECOVERY_ROUNDS=<N>` now limits consecutive recovery rounds without any complete flow, not total recovery rounds. A flow completion resets the budget; the default and formal-runner value is 65536. Event-loop detection is controlled by `HTSIM_STALL_CHECK_EVENTS` and `HTSIM_STALL_NO_PROGRESS_CHECKS`.
- Dense scale-table workloads generated from the requested model table are `my_workloads/H100-gpt_22B-world_size256-tp8-dp4-pp8-gbs384-mbs1-seq2048-interleaved3.txt` and `my_workloads/H100-gpt_175B-world_size1024-tp8-dp16-pp8-gbs1536-mbs1-seq4096-interleaved3.txt`.
- The formal sequential Dense `spray_plb` FCT batch in worktree `/home/zty/Topo/worktrees/SimAI_TyKuro9-htsim-next`, branch `codex/htsim-next-20260710`, was stopped for the HPN livelock fix. Preserved output is `experiments/htsim_results/csv/htsim_dense_scale_table_spray_plb_final_recovery_20260710_191503`: 256 Meta completed with JCT `8346954.840 us`, 36,021,921 FCT rows, and 1267 EndToEnd rows; the HPN case was stopped without JCT after all 1262 layers were issued.
- The earlier HPN formal process was started before route-bottleneck pacing was rebuilt and therefore used the older loaded binary. It exited 2 after 5:20:47 with 36,022,369 FCT rows and no JCT. The source-rate and reorder-buffer changes are HTSim-only; no file under the NS-3 backend or NS-3 frontend was changed.
- The HPN 256 full-workload `spray_reps` run under `experiments/htsim_results/csv/htsim_dense256_hpn_spray_reps_route_rate_20260711_183903` was stopped after all 1262 layers were issued but the final ReduceScatter and AllGather made no FCT progress. It preserved 36,019,645 FCT rows but has no JCT. The receiver reorder-buffer fix was verified on the 256 MiB HPN core reproducer: exit 0, 12 EndToEnd rows, 58,624 FCT rows, JCT `165421.928 us`, 2:00.36 wall time, and no stall handoff or final-recovery rounds.
- The repaired full HPN `spray_reps` run is `experiments/htsim_results/csv/htsim_dense256_hpn_spray_reps_reorder_buffer_20260711_222253`. A waiting tmux queue named `htsim_dense256_spray_reps_remaining_20260712_000511` will gzip its FCT after successful completion, then sequentially run Meta, DeepSeek, Zcube, RO, and ROFT under `experiments/htsim_results/csv/htsim_dense256_spray_reps_remaining_reorder_buffer_20260712_000511`, compressing each completed FCT before starting the next case.
