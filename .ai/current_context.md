...
- HTSim `spray_plb` now keeps normal-stage tail RTO disabled and uses an ACK-gated final-drain recovery path. Only after ASTRA has no progress and the HTSim event queue is empty, each unfinished flow retransmits its oldest unacknowledged packet; this avoids resetting the whole unacknowledged suffix during normal congestion. Control it with `HTSIM_FINAL_DRAIN_RECOVERY=0|1` and `HTSIM_FINAL_DRAIN_RECOVERY_ROUNDS=<N>`; the default round limit is 32768.
...
- The formal sequential 256/1024 Dense `spray_plb` FCT batch is running in worktree `/home/zty/Topo/worktrees/SimAI_TyKuro9-htsim-next`, branch `codex/htsim-next-20260710`, tmux session `htsim_dense_scale_plb_fct_final_20260710`. Output is `experiments/htsim_results/csv/htsim_dense_scale_table_spray_plb_final_recovery_20260710_191503`; cases run six 256 topologies followed by six 1024 topologies.
