#!/usr/bin/env bash
# GPT-22B Dense @ 256 GPU — FlowSim 半速拓扑（非 NVSwitch 链路带宽减半）
#
# 用法:
#   cd /home/zty/Topo/SimAI_TyKuro9
#   python3 scripts/generate_256_half_topos.py   # 首次需生成半速拓扑
#   bash run_256dense_half_flowsim.sh              # 跑全部拓扑
#   bash run_256dense_half_flowsim.sh Meta HPN     # 只跑指定拓扑
#
# 环境变量:
...
TOPO_DIR="${ROOT}/mytopo/256-half"
OUT="${ROOT}/experiments/flowsim_results/256-half"
THREADS="${FLOWSIM_THREADS:-16}"

# name|topo_file|out_subdir|log_file
ALL_CASES=(
  "Meta|Meta_Topo_256g_8gps_400Gbps_A100|MetaDense|Update-256-halfgpu_22B_Meta256Dense_flowsim.log"
  "HPN|AlibabaHPN_256g_8gps_DualToR_DualPlane_200Gbps_H100|HPNDense|Update-256-halfgpu_22B_HPN256Dense_flowsim.log"
  "DeepSeek|DeepSeek_256g_8gps_p16a0.5_400Gbps_H800|DeepSeekDense|Update-256-halfgpu_22B_DeepSeek256Dense_flowsim.log"
  "Zcube|Zcube_n16_k2_256g_8gps_200Gbps_H100|ZcubeDense|Update-256-halfgpu_22B_Zcube256Dense_flowsim.log"
  "RO|RailOnly_256g_8gps_p16a0.5_400Gbps_H100|RODense|Update-256-halfgpu_22B_RO256Dense_flowsim.log"
  "ROFT|ROFT_256g_8gps_p16a0.5_400Gbps_H100|ROFTDense|Update-256-halfgpu_22B_ROFT256Dense_flowsim.log"
)
...
RunOne() {
  ...
  if [[ ! -f "${topo_path}" ]]; then
    ...
    echo "Run: python3 scripts/generate_256_half_topos.py" >&2
    exit 1
  fi

  mkdir -p "${OUT}/${out_sub}"

  echo "========== FlowSim ${name} Dense 256-half =========="
  "${FLOWSIM}" -t "${THREADS}" -w "${WL}" -n "${topo_path}" -o "${OUT}/${out_sub}/" \
    2>&1 | tee "${OUT}/${log_name}"
  echo "Done: ${OUT}/${out_sub}/EndToEnd.csv"
  echo "========== FlowSim ${name} Dense 256-half finished =========="
}
...
bash "${ROOT}/experiments/flowsim_256/mkdir_dense_half_outputs.sh"
...
echo "全部选定拓扑 FlowSim Dense 256-half 已完成。"
